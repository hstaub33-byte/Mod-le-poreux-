...
\varepsilon_g D_i^{\text{eff}} \frac{d^2 \avg{C_i}^l}{dz^2} 
= \frac{\nu_i a_v k(\eta)}{nF} \avg{C_i}^l
\end{equation}

\subsection*{Module de Thiele macroscopique}

En adimensionnant avec $l_e$ et $\avg{C_i}^l$ :
\begin{equation}
\frac{d^2 \avg{C_i}^l}{dz^{*2}} = \underbrace{\frac{\nu_i a_v k(\eta) l_e^2}{nF \varepsilon_g D_i^{\text{eff}}}}_{\phi^2_{\text{macro}}} \avg{C_i}^l = \phi^2_{\text{macro}} \avg{C_i}^l
\end{equation}
...
\begin{equation}
\frac{d^2 \avg{C_i}^l}{dz^{*2}} = \phi^2_{\text{macro}} \avg{C_i}^l
\end{equation}
...
\begin{equation}
\boxed{\avg{C_i}^l(\zeta) = C_i^0 \frac{\cosh(\phi_{\text{macro}}(\zeta + 1))}{\cosh(\phi_{\text{macro}})}}
\end{equation}

\subsection*{Démonstration de la solution analytique}

On cherche une solution de la forme $\avg{C_i}^l = e^{R\zeta}$, ce qui donne :
...
On a donc :
\begin{equation}
\avg{C_i}^l = A e^{\phi\zeta} + B e^{-\phi\zeta}
\end{equation}
...
\begin{align}
\avg{C_i}^l &= A(\cosh + \sinh) + B(\cosh - \sinh) \notag\\
...
\begin{itemize}
\item $\left.\dfrac{d\avg{C_i}^l}{d\zeta}\right|_{\zeta=-1} = 0 \quad \Rightarrow \quad C\phi\sinh(-\phi) + D\phi\cosh(-\phi) = 0 \quad \Rightarrow \quad D = C\dfrac{\sinh(\phi)}{\cosh(\phi)} = C\tanh(\phi)$

\item $\avg{C_i}^l(0) = C_i^0 \quad \Rightarrow \quad C\cosh(0) + D\sinh(0) = C_i^0 \quad \Rightarrow \quad C = C_i^0$
\end{itemize}

Soit :
\begin{align}
\avg{C_i}^l &= C_i^0\left[\cosh(\phi\zeta) + \tanh(\phi)\sinh(\phi\zeta)\right] \notag\\
...
\begin{align}
\dot{r}_{\text{réel}} &= \int_{-1}^{0} k_v \avg{C_i}^l(\zeta)\, d\zeta
