...
représente l'échange à l'interface fluide/solide. Le flux normal étant nul
sur la partie non catalytique de l'interface, seule la zone active
$A_{sf}^{\mathrm{cat}}$ contribue à cet échange. On introduit la vitesse de
réaction interfaciale $R_i$ définie par :
\begin{equation}
...
production algébrique de l'espèce $i$,
...
introduit l'aire interfaciale \emph{catalytique} spécifique volumique :
\begin{equation}
...
La distinction entre interface géométrique et interface catalytique
n'intervient que dans le terme source réactionnel. Pour le transport des
espèces, l'interface complète $A_{sf}$ reste imperméable, qu'elle soit
catalytique ou non : les problèmes de fermeture et les coefficients
effectifs qui en découlent ($D_i^{\mathrm{eff}}$, $D_i^{\mathrm{eff,mig}}$,
$\sigma^{\mathrm{eff}}$, $\varepsilon^{\mathrm{eff}}$) demeurent donc
inchangés et continuent de porter sur $A_{sf}$.
...
est très petit devant le temps caractéristique macroscopique $l_e$. 
